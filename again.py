...
class NessusScanner:
    ...
    def get_scan_templates(self):
        ...
        return self.make_request("/editor/scan/templates")

    def get_scan_policies(self):
        """Get available scan policies"""
        return self.make_request("/policies")
    ...
    def get_scan_results(self, scan_id, export_format="nessus"):
        """
        Get scan results
        ...
        response = self.make_request(f"/scans/{scan_id}/export", method="POST", data=data)
        ...
        while status != "ready" and checks < max_checks:
            time.sleep(2)
            status_response = self.make_request(f"/scans/{scan_id}/export/{file_id}/status")
    ...
    def delete_scan(self, scan_id):
        """Delete a scan by ID"""
        response = self.make_request(f"/scans/{scan_id}", method="DELETE")
...
def show_help():
    ...
    print("\nUsage:")
    print(f"  python3 {sys.argv[0]} <command> [options] [--debug]")
    print("\nCommands:")
    print("  configure          - Set up connection and authentication")
    print("  test_connection    - Test connection to Nessus server")
    ...
    print("\nExamples:")
    print(f"  python3 {sys.argv[0]} configure")
...
def main():
    ...
    if command == "list":
        ...
        scanner.get_scan_results(scan_id, export_format)
